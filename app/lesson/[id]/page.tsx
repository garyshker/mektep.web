...
const LABEL_KEYS: Record<string, 'label_mc' | 'label_type' | 'label_tap' | 'label_word' | 'label_match' | 'label_clock' | 'label_tf' | 'label_pairs' | 'label_equation' | 'label_addsub'> = {
  mc: 'label_mc', type: 'label_type', tap: 'label_tap',
  word: 'label_word', match: 'label_match', clock: 'label_clock',
  tf: 'label_tf', pairs: 'label_pairs', equation: 'label_equation', addsub: 'label_addsub',
}
    ...
    <div className="min-h-screen flex flex-col overflow-x-hidden" style={{ background: 'var(--background)' }}>
      ...
      <main className="flex-1 flex flex-col px-4 pt-2 gap-4 w-full min-w-0 max-w-full lg:max-w-2xl lg:mx-auto" style={{ paddingBottom: feedbackHeight + 24 }}>
        <div className="bg-card rounded-3xl px-5 py-5 shadow-[var(--shadow-md)] w-full min-w-0 max-w-full overflow-x-hidden">
          ...
          {q?.kind === 'addsub' && q.nl && (
            <NumberLineSolver key={idx} a={q.nl.a} op={q.nl.op} b={q.nl.b} />
          )}
