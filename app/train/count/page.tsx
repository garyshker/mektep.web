...
// Grade-1, step 1: count 1–10 in a ten-frame and pick the number.
...
export default function CountTrainer() {
  ...
  const finishProblem = (wasCorrect: boolean) => { setTotal(c => c + 1); void logTrainerAttempt(supabase, 'g1_count', wasCorrect); rnd.conclude(wasCorrect, newProblem) }
  ...
  if (ended) {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 text-center" style={{ background: 'var(--background)' }}>
        ...
        <h2 className="text-2xl font-display font-black text-foreground mb-1">{t('train_count_title', lang)}</h2>
    ...
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--background)' }}>
      <header className="px-4 pt-5 pb-3 flex items-center gap-3 max-w-md mx-auto w-full">
        ...
        <div className="flex-1">
          <h1 className="font-display font-black text-foreground text-base leading-tight">🔢 {t('train_count_title', lang)}</h1>
      ...
      <main className="flex-1 flex flex-col px-4 pt-2 gap-5 max-w-md mx-auto w-full">
        ...
        <div className="bg-card rounded-3xl px-4 py-7 shadow-[var(--shadow-md)] flex flex-col items-center gap-4">
          <p className="text-sm font-bold text-muted-foreground">{t('count_q', lang)}</p>
          <TenFrame n={n} />
          {status !== 'idle' && (
            <p className="text-4xl font-display font-black tabular-nums animate-mk-pop"
              style={{ color: status === 'right' ? 'var(--success)' : 'var(--foreground)' }}>{n}</p>
          )}
        </div>
          ...
          {options.map(opt => {
            ...
            return (
              <button key={opt} onClick={() => pick(opt)} disabled={status !== 'idle'}
                className={`pop-btn rounded-[var(--radius)] py-6 border-2 font-display font-black text-4xl tabular ${status === 'right' && isAns ? 'animate-mk-pop' : ''}`}
