...
// Grade-2: division as equal sharing — the inverse of the equal-groups
// multiplication, on the same picture. `total` items shared fairly into
// `groups` plates → how many in each. Two phases: count one plate (concrete
// sharing), then see it written as total ÷ groups.
...
function buildOptions(answer: number): number[] {
  ...
  while (s.size < 4 && guard++ < 60) {
    const d = answer + ri(-2, 3)
    if (d >= 1 && d <= 10 && d !== answer) s.add(d)
  }
...
export default function ShareTrainer() {
  const router = useRouter()
  const supabase = createClient()
  const lang = useLang()

  const [p, setP] = useState<{ groups: number; each: number; total: number } | null>(null)
  const [phase, setPhase] = useState<'share' | 'write'>('share')
  ...
  const finishProblem = (wasCorrect: boolean) => { setTotal(c => c + 1); void logTrainerAttempt(supabase, 'g2_share', wasCorrect); rnd.conclude(wasCorrect, newProblem) }
  ...
  if (ended) {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 text-center" style={{ background: 'var(--background)' }}>
        ...
        <h2 className="text-2xl font-display font-black text-foreground mb-1">{t('train_share_title', lang)}</h2>
    ...
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--background)' }}>
      <header className="px-4 pt-5 pb-3 flex items-center gap-3 max-w-md mx-auto w-full">
        ...
        <div className="flex-1">
          <h1 className="font-display font-black text-foreground text-base leading-tight">➗ {t('train_share_title', lang)}</h1>
      ...
      <main className="flex-1 flex flex-col px-4 pt-2 gap-4 max-w-md mx-auto w-full">
        ...
        <div className="bg-card rounded-3xl px-4 py-6 shadow-[var(--shadow-md)] flex flex-col items-center gap-3">
          <p className="text-sm font-bold text-muted-foreground text-center">
            {phase === 'share'
              ? `${p.total} ${t('share_into', lang)} ${p.groups} — ${t('share_each_q', lang)}`
              : t('groups_write_q', lang)}
          </p>
          <EqualGroups groups={p.groups} each={p.each} tone="b" />

          {phase === 'write' && (
            <>
              <p className="text-3xl font-display font-black tabular-nums leading-none mt-1">
                {p.total} <span style={{ color: 'var(--accent-deep)' }}>÷</span> {p.groups}
