...
// Grade-1 math: add & subtract within 20, shown with counting sticks.
type StP = { a: number; b: number; op: '+' | '-'; answer: number }
const ri = (lo: number, hi: number) => lo + Math.floor(Math.random() * (hi - lo + 1))

function genProblem(): StP {
  if (Math.random() < 0.55) {                 // addition (foundational, a touch more often)
    const answer = ri(2, 20)
    const a = ri(1, answer - 1)
...
export default function SticksTrainer() {
  const router = useRouter()
  const supabase = createClient()
  const lang = useLang()

  const [problem, setProblem] = useState<StP | null>(null)
  ...
  if (ended) {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 text-center" style={{ background: 'var(--background)' }}>
        ...
        <h2 className="text-2xl font-display font-black text-foreground mb-1">{t('train_sticks_title', lang)}</h2>
    ...
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--background)' }}>
      ...
      <header className="px-4 pt-5 pb-3 flex items-center gap-3 max-w-md mx-auto w-full">
        ...
        <div className="flex-1">
          <h1 className="font-display font-black text-foreground text-base leading-tight">🪵 {t('train_sticks_title', lang)}</h1>
      ...
      <main className="flex-1 flex flex-col px-4 pt-2 gap-4 max-w-md mx-auto w-full">
        ...
        <div className="bg-card rounded-3xl px-4 py-6 shadow-[var(--shadow-md)] flex flex-col items-center gap-4">
          <p className="text-sm font-bold text-muted-foreground">{t('sticks_count_q', lang)}</p>
          <div className="flex items-center justify-center flex-wrap gap-x-3 gap-y-3 w-full">
            <CountingSticks n={problem.a} tone="a" />
            <span className="text-3xl font-display font-black" style={{ color: 'var(--accent-deep)' }}>{opG}</span>
            <CountingSticks n={problem.b} tone="b" faded={problem.op === '-'} />
          </div>
          <div className="flex items-center gap-2 text-4xl font-display font-black tabular-nums">
            <span className="text-muted-foreground">=</span>
            <span style={{ color: status === 'right' ? 'var(--success)' : 'var(--foreground)' }}>
