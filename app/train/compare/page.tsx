...
export default function CompareTrainer() {
  ...
  // Count each tower first — the sign follows from the two numbers, so the
  // closing principle is the one thing worth remembering.
  const hintSteps: HintStep[] = [
    { ask: t('hint_cmp_a', lang), answer: pair.a, lo: 1, hi: 10 },
    { ask: t('hint_cmp_b', lang), answer: pair.b, lo: 1, hi: 10 },
  ]
      ...
      <main className="flex-1 flex flex-col px-4 pt-2 gap-5 max-w-md mx-auto w-full">
        ...
        {hint && (
          <HintScaffold lang={lang} onClose={() => setHint(false)}
            principle={t(pair.a === pair.b ? 'hint_cmp_equal_rule' : 'hint_cmp_rule', lang)}
            steps={hintSteps} />
        )}
